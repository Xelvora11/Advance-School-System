<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-wrap items-center justify-between gap-4">
            <div>
                ...
                <h1 class="mt-1 text-2xl font-bold text-[#0B1F3A]">Expenses</h1>
                <p class="mt-1 text-sm text-gray-600">Record operational expenses and review outgoing transactions.</p>
            </div>
            <a href="{{ route('accounts.exports.expenses', request()->query()) }}" class="app-button app-button-light">
    ...
    <div class="mx-auto max-w-7xl space-y-6 px-4 sm:px-6 lg:px-8">
        ...
        <div class="grid gap-6 xl:grid-cols-3">
            <form method="POST" action="{{ route('accounts.expenses.store') }}" class="app-card grid gap-3 p-5">
                @csrf
                <div>
                    <h2 class="font-bold text-[#0B1F3A]">Add Expense</h2>
                    <p class="mt-1 text-sm text-gray-500">Manual expense entries reduce the running cash balance.</p>
                </div>
                            ...
                            <tbody class="divide-y divide-gray-100">
                                @forelse ($transactions as $transaction)
                                    <tr>
                                        ...
                                        <td class="text-right font-bold text-red-700">PKR {{ number_format((float) $transaction->expense_amount, 2) }}</td>
                                    </tr>
                                @empty
                                    <tr><td colspan="5" class="py-8 text-center text-gray-500">No expense transactions found.</td></tr>
                                @endforelse
                            </tbody>
                        </table>
                    </div>
